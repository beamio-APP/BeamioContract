...
async function main() {
  ...
  const factoryAddress = "0xabc1167197F6D3Be689765A774b1A3A5B4e79D1D"; // Base Sepolia 修复后的 Factory
  ...
  console.log("调试 createAccountFor 调用...");
  ...
  // 检查 Factory.getAddress（使用直接调用）
  console.log("\n检查 Factory.getAddress...");
  try {
    const iface = factory.interface;
    const data = iface.encodeFunctionData("getAddress", [TARGET_EOA, nextIndex]);
    const result = await ethers.provider.call({
      to: factoryAddress,
      data: data
    });
    const decoded = iface.decodeFunctionResult("getAddress", result);
    const computedAddress = decoded[0];
    ...
    if (code !== "0x" && code.length > 2) {
      console.log("⚠️  地址已有代码，Factory.createAccountFor 应该会注册它");
    }
  } catch (error: any) {
    console.error("❌ getAddress 调用失败:", error.message);
  }
  
  // 尝试使用 encodeFunctionData 调用 createAccountFor
