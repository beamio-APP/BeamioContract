...
async function main() {
  ...
  const deployerAddr = data.contracts?.beamioUserCardDeployer?.address;

  if (!cardFactoryAddr || !deployerAddr) {
    console.error("部署文件中缺少 beamioUserCardFactoryPaymaster 或 beamioUserCardDeployer");
    process.exit(1);
  }
  ...
  const deployer = new ethers.Contract(deployerAddr, deployerAbi, ethers.provider);

  const [factoryDeployer, deployerFactory, factoryOwner, deployerOwner, factoryCode, deployerCode] =
    await Promise.all([
      factory.deployer(),
      deployer.factory(),
      factory.owner(),
  ...
  if (!deployerPointsToFactory) {
    ...
    console.log();
    console.log("修复：由 Deployer owner 调用 setFactory(Card Factory)：");
